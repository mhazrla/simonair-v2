<x-perfect-scrollbar as="nav" aria-label="main" class="flex flex-col flex-1 gap-4 px-3">
    ...
    <x-sidebar.dropdown title="Logdata" :active="Str::startsWith(
        request()
            ->route()
            ->uri(),
        'buttons',
        ...
        @foreach ($dashboard as $alat)
            <x-sidebar.sublink title="{{ $alat->dashboard->nama_alat }}" href="{{ route('log', $alat->dashboard->id) }}"
                :active="request()->routeIs('log', $alat->dashboard->id)" />
        @endforeach
